...
5. Subject-presence checkpoints (usually `*_present`) gate the whole rule — judge them **strictly against the rule's specific subject type**. `satisfied` only if the subject of exactly this type is confirmed (e.g. `vertical_opening_present` requires a vertical/wall-type opening — a horizontal floor opening does NOT satisfy it; a slab edge is not an opening; a stair edge is not a roof edge). If the visible subject is of a similar but different type, mark the checkpoint `violated` (this rule's subject is absent) and name the actual type in `visible_evidence`. `not_visible` only when presence is genuinely unconfirmable (limited view, occlusion, blur) — never mark "didn't see it" as `violated`, and never map a different subject type onto this rule.
